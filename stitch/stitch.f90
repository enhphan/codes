...
open(unit=11, file='catx.out')!The raw catting of fragments. used to test read
open(unit=13, file='working.rxp')
100 format(A180)
do
 read(10,100,iostat=io)fragpath !read in the absolute path in fragments.in
  if(io1 < 0) then
  write(*,*) "No fragment paths were provided. Bye bye."
  exit
   ...
   do
    ...
    if (io2 ==0) then
     write(13,100)testline !write out the lines to working.rxp
     write(11,100)testline !write to catx.out
     rxpline=testline
    else 
     rxpline(33:40)='-9.9999'!write non-sense Xi on last line of fragment's rxp
     write(13,100))rxpline !write the modded line to catx.out
     write(11,100)rxpline!write to catx.out
     exit
    endif
   enddo
  endif
